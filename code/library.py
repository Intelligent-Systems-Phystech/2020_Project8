...
def drow_data_cluster(X_test, prediction_vector, List_of_x, T):    
    Text_size = 56
    ...
    for t in np.unique(prediction_vector):
        ...
        _ = plt.plot(List_of_x[ind]+T, X_test[2*T:X_test.shape[0]-T][ind], 'o', color = color[t], label = 'Type ' + str(t + 1))
