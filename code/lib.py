...
def return_h(input, i, T = 10):
    return input[i-T:i, :].transpose()

def return_U(input, i, T = 10):
    return input[i-T:i+T, :].transpose()

def return_phase_track(input, T = 10):
    """
    input has a shape [batch_size, time_len, 1]
    """

    phase_track = np.zeros([input.shape[1], input.shape[0] - T, T])
    
    for i in range(T, input.shape[0]):
        phase_track[:, i-T , :] = return_h(input, i, T)
    
    return phase_track
    

def local_basis(phase_track, m = 2, T = 20, N_components = 2, discrete = 1): # компоненты в точках T,T+discrete.. max-T-discrete
    ...
    for n in range(T, result_pca_1.shape[0] - T, discrete):
        if n-T >- 0:
...
def local_basis_multi(phase_track, m = 2, T = 20, N_components = 2, discrete = 1):
    List_of_basis = []
    List_of_basis_s = []
    List_of_basis_c = []
    for i in range(phase_track.shape[0]):
        List_of_basis_vector, List_of_basis_vector_s, List_of_basis_vector_c = local_basis(phase_track[i], T = T,
                                                                           N_components = N_components,  discrete = discrete)
        List_of_basis.append(List_of_basis_vector)
        List_of_basis_s.append(List_of_basis_vector_s)
        List_of_basis_c.append(List_of_basis_vector_c)
    List_of_basis = np.array(List_of_basis)
    List_of_basis_s = np.array(List_of_basis_s)
    List_of_basis_c = np.array(List_of_basis_c)
    
    List_of_basis = np.transpose(List_of_basis, axes = (1,0,2,3)).reshape(List_of_basis.shape[1],-1,List_of_basis.shape[3])
    List_of_basis_s = np.transpose(List_of_basis_s, axes = (1,0,2)).reshape(List_of_basis_s.shape[1],-1)
    List_of_basis_c = np.transpose(List_of_basis_c, axes = (1,0,2)).reshape(List_of_basis_c.shape[1],-1)
    return List_of_basis, List_of_basis_s, List_of_basis_c
